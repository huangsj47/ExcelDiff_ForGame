...
- 模型**初始只拿到变更清单**（提交信息 + 文件路径），**diff 正文要它自己点名索取**：
  四个只读工具（`commit_detail` / `file_diff` / `file_content` / `read_reference`）
  每轮只点名 4~8 个最可疑的文件 —— 周版本一次可能改几百个文件，全塞进提示词等于把
  预算花在无关的文件上。拿到的证据再按门槛（严重度 / 置信度 / 条数）过滤后落库
- 增量累积：第二次起带上「上次报过的问题」并要求标注「仍成立 / 已修复 / 已被推翻」
- 子代理模式（**默认关**，仅周版本）：把九个检查维度分组交给几个分片代理分头深挖
  （默认 3 个、最多 6 个），再由主代理汇总。所有分片的请求共享同一段**逐字节相同的前缀**
  （系统提示词 + 整份变更清单），所以第一个分片写下的缓存后面每个都能命中；
  哪个分片没跑成会在报告里点名写成「信息缺口」，不会长成「没问题」
