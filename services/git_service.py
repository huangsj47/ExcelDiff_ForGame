...
import git
from services.commit_ordering import order_commits_by_topology
...
class GitService:
    ...
    def order_commits_by_topology(self, commit_ids, timeout=180):
        ...
        实现与理由（同刻提交、数据库行序为什么不等于真实先后、为什么用一次
        `git rev-list` 而不是逐对 `--is-ancestor`）见 `services/commit_ordering.py`。
        留在本类里只因为它要本实例的 `_run_git_command` 与 `local_path`。
        """
        return order_commits_by_topology(self, commit_ids, timeout=timeout)
