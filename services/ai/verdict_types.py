...
#
# **这一条现在写进了契约**：`references/risk-grading.md` §4 要求风险清单每条以 `R#` 开头、
# 编号整节连续唯一（模块之间不重开），理由就是它 —— 那一段是这把锁的另一半。改编号写法
# 顺着改这里，两边一起；只改一边的表现是**静默**的：正则匹配落空 → 每条裁决都显示成
# 「正文未编号」（`verdict.assign_body_labels` 找不到就不写，见 `:238`）。
