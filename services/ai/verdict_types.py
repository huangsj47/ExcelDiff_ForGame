...
#
# **这一条写进了契约的两处，改编号写法要两处一起改**：
#   * `SKILL.md`「后五节」那条 bullet：**每条以 `R#` 开头、整节连续唯一、换模块不重开**
#     ——2026-09-25 加的，因为它原先只在下面那一份 reference 里，而 references 是
#     **按需 `read_reference`**、不进提示词：真机 run 69 实测模型一次都没取过那一份，
#     正文于是 0 个 `R#`，裁决全部落不回正文（这条教训值得记着，别再把硬约束只写进 references）；
#   * `references/risk-grading.md` §4：判据与例子的取材来源。
# 它们与下面这把正则互为另一半。只改一边的表现是**静默**的：正则匹配落空 → 每条裁决
# 都显示成「正文未编号」（`verdict.assign_body_labels` 找不到就不写，见 `:238`）。
