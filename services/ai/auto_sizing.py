...
# `ROUNDS_PER_SHARD`（10）轮各花掉一份保守预留上界 128,500，**再**留一份给下一轮
# （11 × 128,500 = 1,413,500），除以 `1 − REPORT_RESERVE_RATIO` 得 1,662,941 ——
# 口径与下面那条用例一致：跑满 10 轮之后，闸门还得放行第 11 轮。300 万是它的近两倍
# （留了「实测单轮比预留贵」的余量：run 3 单轮实花 202,283，而它的预留约 140,000）。这条关系有测试钉住
