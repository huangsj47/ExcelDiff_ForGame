...
# 旧结论的**收口**规则：模型说「这一条修好了 / 被推翻了」，必须走结构化字段，不能只在
# 正文里写一句。
#
# ## 一处产地、两个投递点（2026-09-25 真机实测）
#
# 这句话原先只写在上面那个摘要抬头里，而那份摘要是**第一轮**的消息、还被
# `<untrusted-data kind="baseline">` 封套包着。实测 run 75：这句话在提示词里、模型也确实
# 取回了**已经改好**的那份服务端代码（`BattleMgr.lua` 在冻结版本上就带着兜底上限），
# 结果它在正文里写「另 3 条本轮复核为**已修复**，不再列入清单」，最终 JSON 里
# `baseline_updates` 是空数组 —— 平台那一节于是照旧记着「2 条仍成立、2 条本轮无结论」，
# 同一份报告两种说法（run 73→74 那个翻转的第三次）。
#
...
#
# ## 2026-09-25 晚订正：上面那句「`baseline_updates` 是空数组」推不出「它没填」
#
# 那个空数组是**平台自己造的**（`protocol.ground_payload` 漏带 `baseline_closures`）。
# run 77 / 78 是反证：清单 5 / 6 条一条状态都没有时引擎会当场补问，而那两轮一次都没补问。
# 上面这段关于**投递位置**的结论不受影响（正文与字段的矛盾是真的、那句提醒也确实被照做了），
# 但「模型不填这个字段」这个推断要划掉。
