...
## 闸门管的是**探索**调用；「交付那一次」在它的定义域之外

三种额度用尽、而手上已经有取证时，平台会**再补一次**「现在出结论」
（`services/ai/wrap_up.py`）—— 那笔钱来自 `report_reserve_tokens`，正是这笔预留的用途。
它**照样按次扣账**（不记的话平台自己报的数就是错的），所以那一刻 `spent` 可能略微越过
上限：这是一个刻意的取舍，不是漏判。分界写在这里，是因为「预算闸门」这个名字很容易被
读成「一次调用都不许多花」——而那会把「交付一份报告」也让出去。
"""
