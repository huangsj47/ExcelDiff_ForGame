...
#
# 为什么要它：**「模型没写」这个判断在 2026-09-25 晚被推翻了**。当时看到的证据是
# 「真机 run 75 / 76 的 `baseline_updates` 是 `[]`」，据此认定它只写正文、不填字段 ——
# 而那个 `[]` 是**平台自己造的**：`protocol.ground_payload` 手抄构造参数表时漏带了
# `baseline_closures`，接地这一步把写好的也扔了。真正的反证是 run 77 / 78：清单里有
# 5 / 6 条、一条状态都没有时引擎会当场补问（`missing_baseline_statuses`），而那两轮
# **一次都没补问** —— 只可能是收下了、又被接地扔掉。
#
# 不过这条通道仍然要改成枚举，理由不依赖上面那个误判：**平台逼得住的字段有先例**
# —— `dimensions` 一次没漏过，因为它是**必填且要逐条枚举**的。所以这里也做成枚举：
# 清单里有几条就要有几条状态，缺了平台当场要求补（见 `protocol.missing_baseline_statuses`
# 与 `engine` 的纠正那一支）。
#
