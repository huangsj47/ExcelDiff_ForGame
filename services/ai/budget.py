...
def truncate_bytes(text: str, max_bytes: int, *, encoding: str = "utf-8") -> tuple[str, bool]:
    ...
    `truncate_text` 量的是**字符数**，而跨节点回传那条路上该量的是**字节数**：落库的是
    `AgentTask.result_summary` 这一列的 JSON。字符数是个会随内容语言漂移的代理指标 ——
    同样 11,000 字，一段中文配表差异是 33,000 字节（**一半额度没用上就停了**），而一段
    ASCII 代码补丁只有 11,000 字节（另一半同样空着）。而「代码 diff 看不到」正是最需要
    这个额度的地方，所以该按字节收敛。

    （这条理由与列的物理上限无关：2026-09 该列已换成 `BigText`，MySQL 上不再是 65,535
    字节的 `TEXT`。「字符数会随语言漂移」这个毛病，列再大也照样存在。）
...
def enforce_budget(
    ...
    **体积判断只看「最终会留下的那些条目」。** 超出条数上限的那些反正会被
    ...
    真实触发路径：模型一次索要 12 个文件（预算本来就允许 12 次），而**当时**的条数上限
    是 8（现已 40，见 `DEFAULT_MAX_ITEMS`；这两个数都改过，别把这段的 8 当成现值）；
...
def looks_like_context_overflow(text: object) -> bool:
    ...
    * **认错（把别的错当成超窗）**：最多多花三次压缩后的调用（按「丢得越少越先试」：先只压
      条目、再连历史一起丢、最后换成收尾提示词；这一轮本来就没条目可压时只剩收尾那一次）
      —— 都被拒的话末尾仍然报失败，而且真实报错原文就在末尾那句话的括号里。
