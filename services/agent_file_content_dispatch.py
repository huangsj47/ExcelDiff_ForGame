...
def request_references(
    ...
    **它是整批，不是前缀范围内的那一批**：Agent 按整批建一次快照索引
    （`ai/reference_index`，含 `MAX_INDEX_FILES` 的预热门槛），范围前缀在它自己的
    `search()` 里筛。只发前缀命中的那一批会让 Agent 端的索引缓存按前缀分叉 ——
    「先问 `scripts/`、再问全局」的第二次查询看到的还是第一个前缀的子集。

    ## 为什么还要带 `total_files`

    它是**这次结论的分母**：本批次里落在这个前缀范围内的文件数。Agent 自己也能数出来
    （它手里是整批），但那个数依赖「平台真的把整批发过来了」这个前提 —— 带过去就是把这个
    前提显式化（`apply_batch_total` 只在两边不一致时把分母抬上去并标 `truncated_files`）。
    分母错一档，模型就会把「没搜到」读成结论：真实历史上出现过的形状是抬头写着
    「240/240，全覆盖了」，而真相是 767 个文件里只看了 240 个。

    这与平台本地那条路（`_search_local` 的 `index.search(prefix=...)` 数出同一个数）必须
    **一致**：同一个周版本在单机与多节点两条部署下印出互相矛盾的覆盖率时，模型正是拿这个数
    决定「能不能说『没有其它引用』」。
    ...
    * **`lines` 承载的是这次检索的签名**，不是行窗口：`_matches` 靠 payload 里的
      ...
      **文件清单那一项不能省成「条数」**：`_find_task` 是在**这个项目+仓库**的最近 80 条
      任务里找，完全不看批次也不看时间（`_recent_tasks`）。省成条数的话，**两个不同周版本、
      改动文件同样多、问的是同一个词**的三个条件一凑齐，签名就一模一样 —— 本周的检索会直接
      命中**上周那条已完成的任务**，把上周的命中清单（路径、行号、那一行的原文）当成本周的
      结果交出去。症状是「模型拿着过期的证据写进本版本报告，而本周新出现的引用一个都搜不到」，
      界面上完全看不出来（额度也不扣，因为根本没派发）。

      `total_files` 也一样要进签名，理由更细一点：它**不影响命中清单**，只影响抬头那句
      覆盖率。少了它，「上周一共有 767 个文件」的那份结果会被本周「一共 300 个文件」的检索
      复用，抬头写成 `240/767` —— 本周的覆盖率被写成了上周的，而模型正是拿这个数决定
      能不能说「本批次的文件里没有别处引用」。
    """
    signature = (
    ...
    return _request_from_agent(
        ...
        extra_payload={
            ...
            # 这次结论的分母：本批次里**落在这个前缀范围内**的文件数（Agent 自己也能从
            # 整批清单里数出来；带过去是把这个前提显式化，不一致时以平台的为准）。
            "total_files": int(total_files or 0),
            # **整批**（不是前缀范围内的那一批）：Agent 按它建快照索引，前缀在查询时筛。
