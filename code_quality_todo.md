...
- [ ] 4. 缩减裸 `except Exception`（先处理 `app.py`、`task_worker_service.py`）
  ...
  - 进展（2026-03-08）：`status_sync_handlers` 的清空确认状态、同步映射查询、同步配置列表与周版本批量确认外层兜底已收敛到 `STATUS_SYNC_*_ERRORS` 明确异常集合，移除该模块剩余裸 `except Exception`。
...
- [ ] 9. 为高复杂函数补充分层单测（边界条件、异常分支、缓存回退）
  ...
  - 进展（2026-03-08）：已新增 `tests/test_status_sync_handlers_exception_narrowing.py`，覆盖清空确认状态失败、同步映射查询失败、同步配置列表失败与周版本批量确认失败回滚分支。
