...
# 版本不匹配的缓存在启动时由 clear_startup_version_mismatch_cache 按这个版本清掉）。
# 仓库里 config.py 还有一份同名字面量，它**目前没有生产读者**（原先读它的
# tasks/cache_cleanup.py 没有任何调用者，已整包删除），所以两份不一致现在只会在
# tests/test_diff_logic_version_single_source.py 上报红，不再造成运行时后果。
# 仍然要求同时改：那一份是版本升级的变更记录，也是仓库外工具可能的读取点。
...
#         三行表头的表里第 2、3 行一直被当成数据行（每次提交都报出几条「表头行被修改」）。
#         现在每张表多出 header_rows/header_stats 两块：表头行照旧逐格显示变更，
#         但不再冒充数据行，stats 也只数数据行。未配置的仓库载荷逐字不变。
#         同一版里还加了「名称行」（Repository.header_name_row）：列名取表头块里的第几行。
#         配 2 的表（第 1 行是大标题、第 2 行才是字段名）修前列头是
#         `道具配置表 / Unnamed: 1 / …`，而只改标题会被报成「某列改名」。
#         它同样不影响未配置的仓库（新列在库里是 NULL ⇒ 名称行 = 1 ⇒ 载荷逐字不变），
#         所以不另起版本号：这一版之后的缓存口径才是权威的。
# 1.19.0：窗口内「新增后删除」（A→D）的两处口径修正（639b726）——
#         （a）删除基线不再取 earliest 的父提交（窗口起点文件还不存在，删除分支
#         拿不到内容），改取窗口内文件仍存在的那条提交，删除前内容第一次可展示；
#         （b）Excel 删除分支基线取不到内容时，不再走通用路径把空字节喂给 pandas
#         （产出「解析失败」假差异并被缓存），改为诚实的删除说明载荷。
#         不升版本号的后果当时已经真实发生：weekly_version_diff_cache 靠同步
#         逐字比对自愈了，但 diff_cache 里冻结的 error 载荷按版本号继续命中，
#         单提交 diff 页永远显示「解析失败」—— 修复在界面上看不见。
