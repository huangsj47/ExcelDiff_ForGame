...
(function (global) {
    ...
    // 分片成员的名字形状（`S1`、`S2`…）。**只有匹配它的才加「分片」这个名词**
    // —— 汇总（「汇总」）与对账轮（`V1`）不是分片，给它们套上「分片」是给读者一个
    // 错的职责（见 `agentText` 的说明）。
    var SHARD_LABEL_RE = /^S\d+$/;
     ...
     *
     * `agent` 是服务端给的**成员名**：分片是 `S1`/`S2`…，汇总是「汇总」，对账轮是 `V1`
     * （见 `services/ai/subagent_tasks` 与 `family_ledger`）。两个都要写「第几个/共几个」，
     * 因为**只看轮次会误读**：「第 1 轮」跑了两分钟，究竟是第一个分片刚起步，还是已经在
     * 汇总了，完全看不出来。
     *
     * ## 「分片」这个名词**只加在分片身上**

     * 原先不管拿到什么名字都拼一句「分片 X」，于是汇总那一轮显示成「分片 主代理 (n+1/n+2)」
     * （名字靠 `index === total` 猜，而 verify 开着、那一轮又被预算跳过时猜不到），
     * 对账轮显示成「分片 V1」。认不出是分片时**不加名词**：宁可只给名字，
     * 也不要给读者一个错的职责。
     */
    function agentText(progress) {
        ...
        if (!name) {
            // 服务端没给名字（老帧 / 落库那份丢过这一列）：按位次猜一个**名字**可以说
            // （末位就是汇总），但**不猜职责前缀** —— verify 开着、那一轮又被预算跳过时
            // 「末位」是汇总、`index < total` 的却不是「主代理」那么简单。
            name = index >= total ? '汇总' : '主代理';
        }
        var prefix = SHARD_LABEL_RE.test(name) ? '分片 ' : '';
        return prefix + name + ' (' + index + '/' + total + ')';
    }
