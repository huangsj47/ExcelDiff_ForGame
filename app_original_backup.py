...
def log_request_info():
    ...
    if request.path.startswith('/admin/'):
        log_print(f"📥 收到请求: {request.method} {request.path}", 'REQUEST', force=True)
        if request.method == 'POST':
            log_print(f"📋 POST请求详情: {request.path}", 'REQUEST', force=True)
...
def check_local_repository_exists(project_code, repository_name, repository_id):
    """检查本地仓库是否存在"""
    import os
    local_path = os.path.join('repos', f"{project_code}_{repository_name}_{repository_id}")
...
def update_repository(repository_id):
    ...
    try:
        ...
        repository.path_regex = request.form.get('file_type_filter')  # 修正字段名
        ...
        if repository.type == 'git':
            ...
            repository.token = request.form.get('token')
            ...
            repository.password = request.form.get('password')
...
def delete_repository(repository_id):
    ...
    local_path = os.path.join('repos', f"{repository.project.code}_{repository.name}_{repository.id}")
