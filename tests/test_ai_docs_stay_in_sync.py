...
def test_the_doc_says_the_numeric_knobs_are_auto_derived():
    ...
    # 取代它们的说明必须在场，且把**片数由什么决定**写出来。
    assert "平台自动推导" in doc
    assert "auto_sizing" in doc, "没指向推导模块的事实源"
    # 「默认 5 个分片」是**预算反解时代的措辞**（2026-09-23 工作包 B 之前的口径：
    # 预算大就开 5 片，正是配置 3 花掉 98 万 token 的来源）。现在片数由快照事实推出、
    # 预算只是上限。留着那句话，读的人会以为分工是固定的一个数。
    assert "默认 5 个分片" not in doc, "还在写「默认 5 个分片」：那是预算反解时代的措辞"
    assert "预算只是" in doc, "没写清「预算只是上限，不是要花掉的目标」"
    assert "变更簇" in doc, "没写清分工是按什么切的"
