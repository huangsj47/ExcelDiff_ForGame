...
#
# 10 → 15（2026-09-20）→ 20（2026-09-21）：这条闸门原先**触发时完全不声不响**（截断只记进
...
# **默认关**（2026-09-23 由「默认开」改回来）。
#
# ## 为什么改回默认关（这不是口味问题，是实测）
#
# 「改动量大到一个模型读不完，所以分几个分片分头深挖」这个前提**只在规模真的很大时成立**。
# 同快照、同模型、同提示词、同价格表的两次实测（docs/AI分析全链路复测与强制改造指引
# -2026-09-23 §1）：
#
# | | 角色/轮次 | 上下文索取 | 总 token | 耗时 | 金标 |
# |---|---:|---:|---:|---:|---:|
# | run 45 多代理 | 7/33 | 111 | 979,910 | 18 分 45 秒 | 2/2 |
# | run 46 单代理 | 1/4 | 11 | 114,333 | 2 分 59 秒 | 2/2 |
#
# 而那次输入只有 **3 个变更文件、1,597 字 diff** —— 多花的 88% token 买的是「多个角色
# 重复审阅同样那 3 个文件」，覆盖率一点没多（两者都 3/3 文件证据）。
#
# ## 它现在管的是「**允许**分工」，不是「**一定**分工」
#
# 分工由 `services/ai/auto_sizing.plan_analysis` 按**冻结快照的事实**决定（文件数、
# 渲染后的 diff 体量、单文件最大值），并且：
#   * 手动手动关掉这一栏 = **强制单代理**，自动规划**不许**把它打开（指引 §3.B）；
#   * 打开它只是「允许」——小批次仍然走单代理（3 个文件的那次就在这一档）。
# 所以管理员主动打开它，是把大版本交给分工，而不是把每一次都变成 n+1 倍调用。
#
# ## 老项目怎么迁移
#
# 平台尚未正式部署，存量行里显式的 `True` **直接按新默认理解**（由管理员主动开启），
# 不为旧数据写兼容层 —— 与「可重建库」的既定口径一致。
DEFAULT_SUBAGENT_ENABLED = False
# 打开之后的默认成员数。1 = 退化成原来的单代理（那时不该走子代理这条路），所以
# 有效范围是 2~6；存 1 也允许（界面上的「关掉」有两处，这里不额外制造一种非法状态）。
#
# 2026-09-23 配置面收敛后这一栏**不再可配**（`endpoint_service.RETIRED_FIELDS`），
# 周版本路径的成员数与每片额度由 `services/ai/auto_sizing.plan_analysis` 按**快照事实**
# 推导（独立变更簇 2~5 个，见那边的 `FAMILY_MIN_MEMBERS` / `FAMILY_MAX_MEMBERS`）；
# 这里只剩两个读者：单提交路径不拆分片、`resolved()` 给已冻结的存量行兜底。
# 默认 3→5 已不再对应任何推导目标（推导值不再读这一列），保留 5 只是**兜底值的形状**
# 与旧行一致，别再把它当成「平台打算开 5 片」的证据。
    ...
    def resolved(self) -> dict:
        ...
        **所有读取路径都必须走这里**。直接读 `config.min_severity` 在老库上会拿到
        ...
        return {
            ...
            # 子代理模式：NULL（老行）读成 `DEFAULT_SUBAGENT_ENABLED`（= 关）与 5。
            # 2026-09-23 改回默认关之后，NULL 与 False 是同一个意思（「没有主动打开」），
            # 所以老行不需要回填 —— 与这一列刚加出来时（默认关）的口径一致。
            "subagent_enabled": (
