...
#
# 两件要知道的事：
#   * 它让**一次周版本分析**的模型调用次数变成 (n+1) 倍（默认 5 个分片 = 6 次调用）。
...
# 周版本路径的分片数由 `services/ai/auto_sizing.py` 推导（目标 5）；这里只剩两个
# 读者：单提交路径不拆分片、`resolved()` 给已冻结的存量行兜底。默认 3→5 与推导
# 目标对齐，避免「推导说 5、兜底说 3」两套数。
